...
// Fetch user data and certificates on component mount
useEffect(() => {
 ...
 try {
  const response = await fetch(`http://localhost:5000/users/data?email=${user.email}`);
  if (!response.ok) {
  ...
  // Check if the certificates array contains the "Failed" string.
  if (userData.certificates && userData.certificates.includes("Failed")) {
  // If it does, set the certificates to an empty array so nothing is displayed.
  setUserCertificates([]);
  } else {
  // Otherwise, map the certificate codes to their full names as before.
  const mappedCerts = userData.certificates.map(certCode => certificateNames[certCode] || certCode);
  ...
  // Set certificates to an empty array on error
  setUserCertificates([]);
 } finally {
  setIsLoading(false);
 }
 ...
 <div className="max-w-4xl mx-auto">
  ...
  {userCertificates.length > 0 ? (
   ...
   >
    ...
    <button
     onClick={() => generateAndDownloadPdf(cert)}
